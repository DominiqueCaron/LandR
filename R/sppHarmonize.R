...
sppHarmonize <- function(sppEquiv, sppNameVector, sppEquivCol, sppColorVect,
  ...
  if (is.na(sppEquivCol)) {
    ...
    if (sppEquivCol %in% names(sppEquiv)) {
      message(paste(
        "Using species listed under", sppEquivCol,
        "column in the 'sppEquiv' table"
      ))
    } else {
      stop(
        "'sppEquivCol' is not a column in 'sppEquiv' or 'LandR::sppEquivalencies_CA'.",
  ...
  if (length(sppMissing)) {
    stop(
      "Species in 'sppNameVector' (", paste(unique(sppMissing), collapse = ", "), ") ",
  ...
  sppNameVectorInSppEquivCol <- if (!identical(sppEquivCol, sppNameConvention)) {
    LandR::equivalentName(sppNameVector,
      df = sppEquiv,
  ...
  if (length(sppMissing)) {
    stop(
      "Species missing (", paste(sppMissing, collapse = ", "), ")",
  ...
  if (isFALSE(all(sppNameVector %in% sppEquiv[[sppEquivCol]]))) {
    warning(
      "Some species in 'sppNameVector' are not in 'sppEquiv[[sppEquivCol]]'.",
  ...
  if (is.null(sppColorVect)) {
    ...
    if (any(grepl("Mixed", names(sppColorVect)))) {
      if (length(sppColorVect) != (length(unique(sppEquiv[[sppEquivCol]])) + 1) ||
        ...
        stop(
          "Length of 'sppColorVect' differs from number species in final 'sppEquiv'.",
    ...
    if (!all(names(sppColorVect)[names(sppColorVect) != "Mixed"] %in% sppEquiv[[sppEquivCol]])) {
      message(
        "'sppColorVect' names do not match 'sppEquivCol' (", sppEquivCol, ")",
        " and will be converted if possible."
      )
      tempNames <- LandR::equivalentName(names(sppColorVect),
        df = sppEquiv,
        column = sppEquivCol
      )
      if (any(is.na(tempNames))) {
        ...
        stop(
          "Could not convert 'sppColorVect' names. The following are missing",
          " from 'sppEquiv' in 'sppEquivCol' (", sppEquivCol, "):",
  ...
  if (isTRUE(vegLeadingProportion > 0) && isTRUE(is.na(sppColorVect["Mixed"]))) {
    stop(
      "'vegLeadingProportion'  is > 0 but there is no 'Mixed' color in 'sppColorVect'. ",
      "Please supply 'sppColorVect' with a 'Mixed' color or set 'vegLeadingProportion' to zero."
    )
  }

  return(list(
    sppEquiv = sppEquiv, sppNameVector = sppNameVector,
