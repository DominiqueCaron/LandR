test_that("prepRasterToMatch works", {
  ...
  studyAreaS <-  terra::vect(terra::geom(studyAreaS), "polygons", crs = originalcrs,
  ...
  studyAreaL <-  terra::vect(terra::geom(studyAreaL), "polygons", crs = originalcrs,
  ...
  rtmE <- terra::rast(terra::ext(c(-105, -104, 55, 56)),
                      res = c(0.01, 0.01), val = 1)

  #warn that rasterToMatchLarge and rasterToMatch are both missing
  expect_warning(
  RTMs <- prepRasterToMatch(studyArea = studyAreaS,
